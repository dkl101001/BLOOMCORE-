...
from .receipts import ReceiptChain, finite_view, invalid_numbers
...
def evaluate(
    ...
    *,
    ...
    response = _response_vector(proposal, veil_active)
    ...
    arithmetic_valid = finite and not derived_residue

    reasons: list[str] = []
    if not finite:
        reasons.append("INPUT.NONFINITE_OR_EMPTY_WINDOW")
    elif not arithmetic_valid:
        reasons.append("AUDIT.NONFINITE_DERIVED_VALUE")
    ...
    hard_stop = (not arithmetic_valid) or (not proposal.truth_signal) or anti_closure or critic_veto
    if hard_stop:
    ...
    result = WeaveResult(
        ...
        wisdom_score=finite_view(wisdom_score) if arithmetic_valid else None,
        veil_pressure=finite_view(veil_pressure) if arithmetic_valid else None,
        response_vector=finite_view(response) if arithmetic_valid else {key: None for key in response},
        numeric_residue=numeric_residue,
    )
    if receipts is not None:
        receipts.append(
            "GOVERNANCE_WEAVE.WITNESS.v2",
            {
                **asdict(result),
                "reason_codes": list(result.reason_codes),
                "input_projection": finite_view({"proposal": {key: value for key, value in asdict(proposal).items() if key != "content"}, "config": asdict(cfg)}),
